...
#         and if it is already on CLOSED, delete the new instance.
    for op in Problem.OPERATORS:
      if op.precond(S): #precond: S has ith neighbour 
        ...
        if new_state in OPEN: 
          if (g[S]+ S.edge_distance(new_state) + h(new_state) < OPEN[new_state]):
            del OPEN[new_state]
            OPEN.insert(new_state, g[S]+ S.edge_distance(new_state) + h(new_state))
          else: 
            del new_state
            continue
        else: #if not (new_state in OPEN):
          OPEN.insert(new_state, g[S] + S.edge_distance(new_state) + h(new_state))
        BACKLINKS[new_state] = S
        g[new_state] = g[S]+ S.edge_distance(new_state)
        TOTAL_COST = g[new_state]
